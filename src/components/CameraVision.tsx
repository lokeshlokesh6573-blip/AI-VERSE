...
export default function CameraVision({ onClose, onCaptureImage, onOCRResult }: CameraVisionProps) {
  ...
  const [spiderVision, setSpiderVision] = useState(false);
    ...
    >
      <div className="absolute top-8 w-full px-8 flex justify-between items-center z-10">
        <div className="flex flex-col">
          <span className="text-red-500 text-[10px] uppercase tracking-[0.3em] font-black animate-pulse">
            {spiderVision ? 'SPIDER VISION ACTIVE' : 'Vision Subsystem'}
          </span>
          <span className="text-white/50 text-xs tracking-widest font-mono">
            {spiderVision ? 'THERMAL_TRACKING_ON' : 'Live Targeting...'}
          </span>
        </div>
      ...
      <div className={`relative w-full max-w-2xl aspect-4/3 rounded-3xl overflow-hidden shadow-2xl transition-all duration-500 border ${
        spiderVision ? 'shadow-red-500/40 border-red-500/50' : 'shadow-blue-500/20 border-white/10'
      } group`}>
        <video
          ref={videoRef}
          autoPlay
          ...
          style={{ filter: spiderVision ? 'sepia(1) hue-rotate(300deg) brightness(1.2) contrast(1.5) saturate(2)' : 'none' }}
        />
      ...
      <div className="absolute bottom-12 flex space-x-6 z-10 w-full justify-center px-4">
        ...
        <button 
          onClick={() => setSpiderVision(!spiderVision)} 
          className={`p-4 rounded-full transition-all backdrop-blur-md border ${
            spiderVision ? 'bg-red-600/40 border-red-500 text-white shadow-[0_0_15px_rgba(220,38,38,0.5)]' : 'bg-white/5 border-white/10 text-white/50'
          }`}
          title="Spider Vision"
        >
